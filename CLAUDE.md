...
- **Renderer services**: `<domain>.service.ts` inside `src/renderer/services/` — these wrap `window.api` calls.
...
- **`src/renderer/components/`**: Group into a feature subfolder when you have 3+ components for the same feature.
- **`src/renderer/pages/`**: One file per top-level route. Nest subfolders for sections with 3+ related pages.
...
```typescript
// src/renderer/services/users.service.ts
...
- Path alias: always use `~/` for imports within `src/renderer/` (e.g. `~/lib/utils`, `~/components/ui/button`).
...
- CSS variables are defined in `src/renderer/app.css` — use them via Tailwind's semantic tokens (`bg-background`, `text-foreground`, `text-muted-foreground`, `border-input`, `bg-destructive`, etc.).
...
- Translation files live in `src/renderer/lib/locales/` (one file per language: `en.ts`, `nl.ts`, etc.).
...
- Before creating a new component, check `src/renderer/components/` for something reusable.
